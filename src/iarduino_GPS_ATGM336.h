...
class iarduino_GPS_ATGM336{																											//
	...
	/**	Пользовательские функции **/																								//
		bool		begin			(HardwareSerial &i				){ _flgTypeSerial=1; _objSerial=&i; return _begin(); }			//	Определяем функцию инициализации парсера				(Параметр:  объект для работы с аппаратным  UART).
		bool		begin			(SoftwareSerial &i				){ _flgTypeSerial=2; _objSerial=&i; return _begin(); }			//	Определяем функцию инициализации парсера				(Параметр:  объект для работы с программным UART).
